// RStan model containing the third inducer exchange model for the Toggle Switch developed by L. Bandiera. Bayesian inference is perfrmed using multiple sets of experimental data instead of just one single set
...
model {
  ...
  for (j in 1:m){
    ...
    ing = SteadyState(to_vector(ivss[1:4,j]), to_vector(theta), pre[1:2,j], x_i); // Calculation of initial guesses for steady state
    Y0[1,j] = ing[1];
    Y0[2,j] = ing[2];
    Y0[3,j] = ing[3];
    Y0[4,j] = ing[4];
