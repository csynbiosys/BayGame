// RStan model containing the second inducer exchange model for the Toggle Switch contained in the Lugagne paper. Bayesian inference is perfrmed using multiple sets of experimental data instead of just one single set
...
generated quantities {
  ...
  for (j in 1:m){
    ...
    for (t in 1:stsl[1,j]){
    logLikelihood_RFP[t,j] = normal_lpdf(RFPmean[t,j]|y_hat[(sts[t,j]+1),3],RFPstd[t,j]);
    logLikelihood_GFP[t,j] = normal_lpdf(GFPmean[t,j]|y_hat[(sts[t,j]+1),4],GFPstd[t,j]);
    };
