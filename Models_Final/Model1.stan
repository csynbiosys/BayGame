...
transformed parameters {
  ...
  theta[2] = exp(((k_L_pm0_raw)*(1.15129254649702))+(-3.50655789731998));
  theta[3] = exp(((k_L_pm_raw)*(1.15129254649702))+(2.30258509299405));
  theta[4] = exp(((theta_T_raw)*(1.15129254649702))+(3.40119738166216));
  theta[5] = exp(((theta_aTc_raw)*(1.15129254649702))+(2.30258509299405));
  theta[6] = (((n_aTc_raw)*(1.25))+(2.5));
  theta[7] = (((n_T_raw)*(1.25))+(2.5));
  theta[8] = exp((((k_T_pm0_raw))*(1.15129254649702))-2.30258509299405);
  theta[9] = exp(((k_T_pm_raw)*(1.15129254649702))+(2.22044604925031e-16));
  theta[10] = exp(((theta_L_raw)*(1.15129254649702))+3.40119738166216);
  theta[11] = exp(((theta_IPTG_raw)*(1.15129254649702))-2.30258509299405);
  theta[12] = ((n_IPTG_raw)*(1.25))+2.5;
  theta[13] = ((n_L_raw)*(1.25))+2.5;
}
...
generated quantities {
  ...
  // ODE solutions as computed in the model section to calculate the Log-Likelihood of the inferences
  for (j in 1:m){
    real ivst[Neq]; // Initial value of the states 
    real y_hat[(tsl[1,j]),Neq];
    int i; // Increasing index for the inputs
    ...
    ing = SteadyState(to_vector(ivss[1:2,j]), to_vector(theta), pre[1:2,j], x_i); 
    Y0[1,j] = preIPTG[1,j];
    Y0[2,j] = ing[1];
    Y0[3,j] = ing[2];
    ssv = integrate_ode_bdf(Toogle_one, Y0[,j],0,toni,theta,pre[1:2,j], x_i, 1e-9, 1e-9, 1e7); 
