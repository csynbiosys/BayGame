...
# Function to plot the 95% confidence intervals plots for the inference results on Model 3. First argument is the name of the experimental
# design to plot, and second argument aditional information that will be included in the title of the plot and the file name. The third argument 
...
# Model3_Function_FakeData.stan is a stan function file necessary to simulate the model for each experimental profile

ConfidenceInterval3 <- function(experiment, aditInfo = "", stanRes){
  
  expose_stan_functions("Model3_Function_FakeData.stan")
  
  past <- as.array(stanRes)
  ...
  png(paste("CI_Mm_GFP_Model3_", fileName, "_Fit", aditInfo,".png", sep=""), width = 6, height = 4, units = 'in', res = 300)
  plot(samplingT, GFPmean, col = "black", pch=20, cex = 0.1, xlab="time (min)", ylab="GFP", 
       main=paste("Results Inference for GFP ", fileName, ", Model 3, ",aditInfo, sep=""), 
  ...
  png(paste("CI_Mm_RFP_Model3_", fileName, "_Fit", aditInfo,".png", sep=""), width = 6, height = 4, units = 'in', res = 300)
  plot(samplingT, RFPmean, col = "black", pch=20, cex = 0.1, xlab="time (min)", ylab="RFP", 
       main=paste("Results Inference for RFP ", fileName, ", Model 3, ",aditInfo , sep=""), 
